...
# Paths (Update to your specific directory structure)
BASE_DIR = "/media/michael/C60D-9867/COCO"

# Processed dataset directories
TRAIN_DIR = f"{BASE_DIR}/processed/train"
...
def load_model():
    ...
    model.fc = torch.nn.Linear(model.fc.in_features, len(os.listdir(TRAIN_DIR)))
...
def test_camera():
    ...
    # Open the webcam
    cap = cv2.VideoCapture(13)  # Replace with the correct camera index
    if not cap.isOpened():
